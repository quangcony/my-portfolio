...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("dnq1310@gmail.com");
    setCopied(true);
  };
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-5">
                {["Nextjs", "Reactjs", "Redux"].map((item, i) => (
                  <span
                    key={i}
                    className="lg:px-5 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-3xl text-center bg-[#1e293b]"
                  >
                    {item}
                  </span>
                ))}
                <span className="lg:px-3 py-4 px-3 rounded-3xl text-center bg-[#1e293b]"></span>
              </div>
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-5">
                <span className="lg:px-3 py-4 px-3 rounded-3xl text-center bg-[#1e293b]"></span>
                {["Nodejs", "MongoDb", "React Native"].map((item, i) => (
                  <span
                    key={i}
